...
int make_pkgsrc_work_setup_desktop(){
    ...
    char commands[] = {"sudo pkg install git xsvc","sudo chmod +x bootstrap.sh","sudo ./bootstrap.sh","source /etc/profile","sudo pkgin install modular-xorg xf86-video-intel mate && touch ~/.xinitrc && echo \"exec mate-session\" >> ~/.xinitrc && startx"};
